...
let flyingTactics = ["jatkaa summassa", "pinnassa", "", "", "", ""]
...
class Aircraft {
    ...
    updateLocation(newLocation) {
        if (Math.random() > 0.9 || this.removed) {
            this.removed = true;
            return
        }
...
for (let i = 0; i < 8; i++) {
    let aircraft = new Aircraft({
        location: {
            x: randomBetweenRange(-500, plane.dimensions[0] + 500),
            y: randomBetweenRange(-500, plane.dimensions[1] + 500)
        },
        direction: randomBetweenRange(0, 360),
        velocity: randomBetweenRange(0.6, 7),
...
const animate = () => {
    ...
    ctx.clearRect(0 - ctx.canvas.width / 2, 0 - ctx.canvas.height / 2, ctx.canvas.width, ctx.canvas.height);
    coordinateLines()
    ctx.stroke()
